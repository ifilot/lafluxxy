 ...
 */
__global__ void construct_laplacian_a(float *df, const float *dfx, const float *dfy) {
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;

    for(int i = index; i < d_ncells; i += stride) {
        df[i] = d_diffcon_a * (dfx[i] + dfy[i]);
    }
}
 ...
 * @param[in]  dfy   pointer to second derivative in y direction
 */
__global__ void construct_laplacian_b(float *df, const float *dfx, const float *dfy) {
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;

    for(int i = index; i < d_ncells; i += stride) {
        df[i] = d_diffcon_b * (dfx[i] + dfy[i]);
    }
}
